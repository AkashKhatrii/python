...
# File class without design pattern
class File:
    def __init__(self, name, size):
        self.name = name
        self.size = size
...
# LinuxFindCommand without design pattern
class LinuxFind:
    def __init__(self):
        self.filters = []
    ...
    def apply_OR_filtering(self, root):
        ...
        while queue:
            curr_root = queue.popleft()
            if curr_root.is_directory:
                queue.extend(curr_root.children)
            else:
                for filter_func in self.filters:
                    if filter_func(curr_root):
                        found_files.append(curr_root)
                        print(curr_root)
                        break
...
def min_size_filter(size):
    def filter_func(file):
        return file.size > size
    return filter_func

def extension_filter(extension):
    def filter_func(file):
...
# Applying filters
greater5_filter = min_size_filter(5)
txt_filter = extension_filter("txt")
